...
const AdminAnalytics = () => {
  ...
  const handleDownload = () => {
    try {
      ...
      doc.setFontSize(12);
      doc.setTextColor(0);
      const dateRangeStr = `${startDate ? startDate.toLocaleDateString() : 'Start'} - ${endDate ? endDate.toLocaleDateString() : 'End'}`;
      doc.text(dateRangeStr, 105, 46, { align: "center" });
    ...
    <div className="min-h-screen bg-[#0a1111] text-white p-4 md:p-8">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-4 w-full md:w-auto">
          ...
          <button onClick={handlePrint} className="flex-1 md:flex-none justify-center flex items-center gap-2 bg-blue-600 px-4 py-2 rounded-lg text-sm whitespace-nowrap">
            <Printer className="w-4 h-4" /> Print
          </button>
          <button onClick={handleDownload} className="flex-1 md:flex-none justify-center flex items-center gap-2 bg-blue-800 px-4 py-2 rounded-lg text-sm whitespace-nowrap">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
        <div className="lg:col-span-3 bg-[#1a2626] p-4 rounded-xl flex flex-col sm:flex-row gap-4 items-center">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <DatePicker
              ...
              className="w-full sm:w-auto bg-[#0a1111] p-2 rounded-lg text-sm border border-white/10 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
            />
        ...
        <div className="bg-[#1a2626] p-4 rounded-xl flex justify-between items-center">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-blue-400">{averageStay.toFixed(1)}</p>
          </div>
        </div>
      </div>
            ...
            {purposeData.map((data, index) => (
              <div key={data.name}>
                <div className="flex justify-between text-xs mb-2">
                  ...
                  <span className="text-blue-400">{data.value}</span>
                </div>
              ...
              {collegeRanking.map((college, index) => (
                <tr key={college.name} className="border-b border-gray-800/50">
                  ...
                  <td className="py-2 md:py-4 px-1 md:px-2">
                    <span className="text-blue-400 font-bold mr-2 truncate max-w-[80px] md:max-w-none block md:inline">{college.name}</span>
                  </td>
                  ...
                  <td className="py-2 md:py-4 px-1 md:px-2">
                    <div className="flex items-center gap-2 md:gap-4">
                      <div className="w-16 md:w-24 h-1.5 bg-[#0a1111] rounded-full overflow-hidden shrink-0">
                        <div className="h-full bg-blue-500 rounded-full" style={{ width: `${college.engagement}%` }}></div>
                      </div>
